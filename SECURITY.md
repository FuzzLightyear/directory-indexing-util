...
> Private vulnerability reporting must be enabled once in the repository's *Settings → Security → Private vulnerability reporting*. If you cannot file an advisory because the feature isn't yet enabled, please open a public issue asking only that it be enabled — withhold any vulnerability details until the private channel is available.
